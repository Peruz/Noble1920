...
def add_grad(skip_ab, skip_cp, skip_mn, skip_mm, electrodes, rec):
    seq = []
    for a in range(1, electrodes[-1] - skip_ab):
        b = a + skip_ab + 1
        for m in range(a + skip_cp + 1, b - skip_cp - 1 - skip_mn, skip_mm):
            n = m + skip_mn + 1
            seq.append([a, b, m, n])
            if rec:
                seq.append([m, n, a, b])
...
def add_grads(seq_list):
    grads = {11:[0, 2, 1], 21: [0, 4, 1], 55: [2, 11, 1]}
    for k, v in grads.items():
        seq = add_grad(k, v[0], v[1], v[2], electrodes, rec=True)
        seq_list.append(seq)
    return(seq_list)
...
if __name__ == "__main__":
    ...
    inj = 0
    inj_df = pd.DataFrame(data=np.zeros(8), index=range(1, 9), columns=['n'], dtype=int)
    for n, g in groups:
        inj += ((len(g) - 1) // 8) + 1
        inj8 = (len(g) - 1) // 8
        if not inj8 == 0:
    ...
    seaborn.distplot(groups.size().reset_index(name='counts')['counts'], bins=range(1, 40))
    plt.show()
    print(inj_df)
